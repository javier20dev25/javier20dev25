...
async function main() {
  ...
  const stats = [
    ...
    `| Estrellas (total) | ${totalStars} |`,
    `| PRs enviados a proyectos ajenos | ${prs.total_count} |`,
    `| Issues abiertos en proyectos ajenos | ${issues.total_count} |`,
    '',
    'Lenguajes principales:',
    ...langEntries.map(([l, n]) => `\`${esc(l)}\` ${bar(n / langTotal)} ${n}`),
  ].join('\n');
